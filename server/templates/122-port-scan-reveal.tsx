...
export const AnimationComponent = () => {
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, width: 1920, height: 1080, overflow: 'hidden', backgroundColor: 'BACKGROUND_COLOR' }}>
      <div style={{ position: 'absolute', top: 60, left: 192, fontSize: 40, fontWeight: 'bold', color: 'TEXT_ON_PRIMARY', letterSpacing: '2px' }}>PORT SCAN ANALYSIS</div>
      ...
      {ports.map((port, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            <div style={{ fontSize: 64, fontWeight: 'bold', color: 'TEXT_ON_PRIMARY', fontFamily: 'monospace' }}>{port.id}</div>
            <div style={{ fontSize: 24, color: isSuccess ? 'PRIMARY_COLOR' : 'SUPPORT_COLOR', marginTop: 16, textTransform: 'uppercase', letterSpacing: '0.12em' }}>{port.status}</div>
          </div>
        );
      })}
    </div>
