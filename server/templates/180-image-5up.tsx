...
// Layout: row 0 → 2 wide cells, row 1 → 3 cells
const cells = [
  { src: IMAGE_URL_1, label: LABEL_1, suggestion: SUGGESTION_1, x: PAD,                       y: TOP,              w: CELL_W_2, h: ROW_H_T, delay: 5  },
  { src: IMAGE_URL_2, label: LABEL_2, suggestion: SUGGESTION_2, x: PAD + CELL_W_2 + GAP,       y: TOP,              w: CELL_W_2, h: ROW_H_T, delay: 12 },
  { src: IMAGE_URL_3, label: LABEL_3, suggestion: SUGGESTION_3, x: PAD,                        y: TOP + ROW_H_T + GAP, w: CELL_W_3, h: ROW_H_B, delay: 19 },
  { src: IMAGE_URL_4, label: LABEL_4, suggestion: SUGGESTION_4, x: PAD + CELL_W_3 + GAP,       y: TOP + ROW_H_T + GAP, w: CELL_W_3, h: ROW_H_B, delay: 26 },
  { src: IMAGE_URL_5, label: LABEL_5, suggestion: SUGGESTION_5, x: PAD + (CELL_W_3 + GAP) * 2, y: TOP + ROW_H_T + GAP, w: CELL_W_3, h: ROW_H_B, delay: 33 },
];
...
const ImageCell: React.FC<{
  ...
  const lblY  = interpolate(rel, [12, 32], [12, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  ...
  return (
    <div style={{ position: 'absolute', left: x, top: y, width: w, height: h, opacity: op, transform: `scale(${sc})`, transformOrigin: 'center' }}>
      <div style={{ position: 'absolute', inset: -2, borderRadius: 10, background: 'rgba(59,130,246,0.10)', filter: 'blur(6px)', opacity: hasImg ? 0.8 : 0.4 }} />
      <div style={{
        ...
        {hasImg ? (
          ...
          <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, boxSizing: 'border-box', textAlign: 'center' }}>
            ...
            {hasSug && (
              <div style={{ color: 'rgba(148,163,184,0.75)', fontSize: 18, fontFamily: 'Inter,system-ui,sans-serif', fontWeight: 500, lineHeight: 1.5 }}>
                {suggestion}
              </div>
            )}
          </div>
        ...
        {hasLbl && hasImg && (
          <div style={{
            ...
            <span style={{ color: '#fff', fontSize: 20, fontWeight: 800, fontFamily: 'Inter,system-ui,sans-serif', display: 'block', textShadow: '0 2px 8px rgba(0,0,0,0.9)' }}>
              {label}
            </span>
          </div>
        )}
        <div style={{ position: 'absolute', top: 10, left: 10, width: 18, height: 18, borderTop: `3px solid ${BLUE}`, borderLeft: `3px solid ${BLUE}`, opacity: 0.7 }} />
        <div style={{ position: 'absolute', bottom: 10, right: 10, width: 18, height: 18, borderBottom: `3px solid ${BLUE}`, borderRight: `3px solid ${BLUE}`, opacity: 0.7 }} />
      </div>
    </div>
  );
};
...
export const AnimationComponent: React.FC = () => {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, backgroundColor: BG, overflow: 'hidden', opacity: bgOp }}>
      ...
      {cells.map((c, i) => (
        <ImageCell key={i} src={c.src} label={c.label} suggestion={c.suggestion} x={c.x} y={c.y} w={c.w} h={c.h} delay={c.delay} />
      ))}
    </div>
  );
};
