...
const VideoGeneratorView: React.FC = () => {
        ...
        const tick = async () => {
            ...
            try {
                ...
                if (data.status === 'completed') {
                    ...
                    setStatus('completed');
                    setLoading(false);
                    saveToHistory(videoId, data.video_url);
                    localStorage.removeItem('heygen_pending_video_id');
                } else if (data.status === 'failed') {
                    clearInterval(pollRef.current!);
    ...
    const saveToHistory = (videoId: string, videoUrl: string) => {
        const avatarName = stockAvatars.find(a => a.id === selectedAvatar)?.name || selectedAvatar || 'Custom';
        const voiceName = stockVoices.find(v => v.id === selectedVoice)?.name || selectedVoice || '—';
        const entry: HistoryEntry = {
            videoId,
            videoUrl,
            scriptSnippet: script.slice(0, 140).trim() + (script.length > 140 ? '…' : ''),
            avatarName,
            voiceName,
            timestamp: Date.now(),
            renderTime: formatTime(elapsedSeconds),
        };
        setHistory(prev => {
            const next = [entry, ...prev.filter(h => h.videoId !== videoId)].slice(0, MAX_HISTORY);
            localStorage.setItem(HISTORY_KEY, JSON.stringify(next));
            return next;
        });
    };

    const deleteFromHistory = (videoId: string) => {
        setHistory(prev => {
