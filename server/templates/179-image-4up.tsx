...
const CELL_W  = (AVAIL_W - GAP) / 2;
const CELL_H  = (AVAIL_H - GAP) / 2;
...
// 2×2 grid: col 0/1, row 0/1
const cells = [
  { src: IMAGE_URL_1, label: LABEL_1, suggestion: SUGGESTION_1, col: 0, row: 0, delay: 5  },
  { src: IMAGE_URL_2, label: LABEL_2, suggestion: SUGGESTION_2, col: 1, row: 0, delay: 12 },
  { src: IMAGE_URL_3, label: LABEL_3, suggestion: SUGGESTION_3, col: 0, row: 1, delay: 19 },
  { src: IMAGE_URL_4, label: LABEL_4, suggestion: SUGGESTION_4, col: 1, row: 1, delay: 26 },
];
...
const ImageCell: React.FC<{
  ...
  return (
    <div style={{ position: 'absolute', left: x, top: y, width: w, height: h, opacity: op, transform: `scale(${sc})`, transformOrigin: 'center' }}>
      <div style={{ position: 'absolute', inset: -2, borderRadius: 14, background: 'rgba(59,130,246,0.10)', filter: 'blur(6px)', opacity: hasImg ? 0.8 : 0.4 }} />
      <div style={{
        ...
        {hasImg ? (
          ...
          <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, boxSizing: 'border-box', textAlign: 'center' }}>
            ...
            {hasSug && (
              <div style={{ color: 'rgba(148,163,184,0.75)', fontSize: 19, fontFamily: 'Inter,system-ui,sans-serif', fontWeight: 500, lineHeight: 1.5 }}>
                {suggestion}
              </div>
            )}
          </div>
        ...
        {hasLbl && hasImg && (
          <div style={{
            ...
            <span style={{ color: '#fff', fontSize: 22, fontWeight: 800, fontFamily: 'Inter,system-ui,sans-serif', display: 'block', textShadow: '0 2px 8px rgba(0,0,0,0.9)' }}>
              {label}
            </span>
          </div>
        )}
