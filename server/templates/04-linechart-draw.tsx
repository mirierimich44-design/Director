...
export const AnimationComponent = () => {
  ...
  const lineProgress = interpolate(frame, [lineStart, lineEnd], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.inOut(Easing.quad) })

  // Chart area dimensions (Safe inside 1600x900)
  const chartW = 1200
  const chartH = 500
  const chartLeft = 200
  const chartTop = 150
  ...
  return (
    <div style={{
      ...
      <div style={{ width: 1600, height: 900, position: 'relative' }}>
        ...
        {/* Header */}
        <div style={{ position: 'absolute', top: 40, left: 40, opacity: entryOp }}>
          <div style={{ color: 'PRIMARY_COLOR', fontSize: 14, fontWeight: 900, letterSpacing: 6, marginBottom: 8 }}>DATA_STREAM_ANALYSIS</div>
        ...
        <svg width={1600} height={900} style={{ position: 'absolute', top: 0, left: 0, zIndex: 1 }}>
          ...
          {visiblePoints.length > 1 && (
            <polyline
              ...
              strokeLinecap="round"
              style={{ filter: 'drop-shadow(0 0 12px PRIMARY_COLOR)' }}
            />
          )}

          {/* Leading Edge Target/Comet */}
          {lineProgress > 0 && lineProgress < 1 && (
            <g transform={`translate(${currentX}, ${currentY})`}>
               <circle cx={0} cy={0} r={12} fill="PRIMARY_COLOR" opacity={0.4} />
               <circle cx={0} cy={0} r={6} fill="#fff" style={{ filter: 'drop-shadow(0 0 10px #fff)' }} />
            </g>
          )}
          ...
          {points.map((p, i) => {
            ...
            return (
              <g key={`dot-${i}`} opacity={pop}>
                <circle cx={p.x} cy={p.y} r={10} fill="BACKGROUND_COLOR" stroke="PRIMARY_COLOR" strokeWidth={3} />
        ...
        {points.map((p, i) => {
          ...
          return (
            <div key={`label-${i}`} style={{
              ...
              opacity: op, color: 'SUPPORT_COLOR', fontSize: 16, fontWeight: 700, fontFamily: 'JetBrains Mono, monospace', letterSpacing: 1
            }}>
              {xLabels[i]}
            </div>
          )
        ...
        {points.map((p, i) => {
          ...
          return (
            <div key={`callout-${i}`} style={{
              position: 'absolute', top: p.y - 60, left: p.x, transform: `translateX(-50%)`,
              opacity: op, backgroundColor: 'rgba(0, 0, 0, 0.7)', backdropFilter: 'blur(10px)',
              borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', padding: '8px 16px',
              boxShadow: '0 10px 20px rgba(0,0,0,0.5)', zIndex: 10, textAlign: 'center'
            }}>
              <div style={{ color: 'PRIMARY_COLOR', fontSize: 20, fontWeight: 900, letterSpacing: 1 }}>
                {lineValues[i]}
              </div>
            </div>
          )
